...
################################################################################
getLogs <- function(logdir, setID,
                    burnIn, lastSample, subSample, toCut = TRUE) {

  ## Collect all logs:
  logfiles <- list.files(paste0(logdir, '/raw'), pattern = '\\.log')
  Log <- lapply(logfiles, cutPrepLog, logdir = logdir, setID = setID,
...
################################################################################
cutLog <- function(logfile, logdir,
                   burnIn, lastSample, subSample,
                   return.log = TRUE, write.cutLog = TRUE) {

  Log <- read.table(paste0(logdir, '/raw/', logfile), header = TRUE)
  cat('\n\n#### cutLog function:', logfile, "\tnrows:", nrow(Log), '\n')

  ## Remove burn-in:
  if(burnIn > 0) Log <- Log[-which(Log$Sample < burnIn), ]

  if(nrow(Log) < 1000) {
    cat('\n#### cutLog function: SKIPPING: LESS THAN 1000 ROWS LEFT AFTER REMOVING BURN-IN\n')
  } else {
    ## Remove final samples:
    if(!is.null(lastSample)) if(any(Log$Sample > lastSample))
      Log <- Log[-which(Log$Sample > lastSample), ]

    ## Subsample:
    Log <- Log[seq(from = 1, to = nrow(Log), by = subSample), ]
    cat("#### cutLog function: Last sample:", max(Log$Sample), '\n')
...
################################################################################
prepLog <- function(oneLog, logfile, runID, setID) {
  ...
  for(migcol in migcols) {
    ...
    #cat('#### prepLog function: Migration pattern:', migpattern, '\n')
    ...
    mutrate.gen_dist <- rnorm(nrow(oneLog), mean = mutrate.gen_mean, sd = mutrate.gen_sd)
  ...
  cat('#### prepLog function: Pops not found:',
  ...
  mlog$migtype.run <- factor(ifelse(grepl('multmig', runID), 'mult',
                                    ifelse(grepl('noMig', runID), 'none', 'single')))
...
################################################################################
##### Step 3: Wrapper function to first apply cutLog(), then prepLog() #####
################################################################################
cutPrepLog <- function(logfile, logdir, setID, runID,
                       burnIn, lastSample, subSample,
                       toCut = TRUE, return.log = TRUE, write.cutLog = TRUE) {

  if(toCut == TRUE) {
    Log <- cutLog(logfile = logfile, logdir = logdir, burnIn = burnIn,
                  lastSample = lastSample, subSample = subSample,
                  return.log = return.log, write.cutLog = write.cutLog)
  }

  if(toCut == FALSE) {
    cat('\n\n#### cutPrepLog function: not cutting log, just reading it in.')
    cat('#### cutPrepLog function: ', logfile, "\tnrows:", nrow(Log))
  ...
  if(!is.null(Log)) {
    ...
    #cat('#### cutPrepLog function: Column names of Log:\n', colnames(Log), '\n')

    Log <- prepLog(oneLog = Log, logfile = logfile, setID = setID)
    return(Log)
  }
}
...
add.cvalue <- function(Log) {
  ...
  ## Tau & theta:
  #if(gentime_prior == FALSE) gentime_sd <- 0
  #if(mutrate_prior == FALSE) mutrate.gen_sd <- 0
