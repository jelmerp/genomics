...
#### SET-UP --------------------------------------------------------------------
cat('#### vcf2loci2a_makelocusbed.R: Starting script.\n\n')
...
## Report:
cat('\n#### vcf2loci2a_makelocusbed.R: Set ID:', setID, '\n')
cat('#### vcf2loci2a_makelocusbed.R: Bedfile dir:', indir_bed, '\n')
cat('#### vcf2loci2a_makelocusbed.R: Bedfiles:', bedfiles, '\n')
cat('#### vcf2loci2a_makelocusbed.R: Number of bedfiles:', length(bedfiles), '\n')
cat('#### vcf2loci2a_makelocusbed.R: Bed output file:', outfile_bed, '\n\n')
cat('#### vcf2loci2a_makelocusbed.R: Max distance within inds:', max.dist.withinInd, '\n')
cat('#### vcf2loci2a_makelocusbed.R: Max distance between inds:', max.dist.betweenInd, '\n')
cat('#### vcf2loci2a_makelocusbed.R: Min element overlap - for locus creation:', min.element.ovl, '\n')
cat('#### vcf2loci2a_makelocusbed.R: Min element overlap - for locus trimming:', min.element.ovl_trim, '\n')
cat('#### vcf2loci2a_makelocusbed.R: Min element size:', min.element.size, '\n')
cat('#### vcf2loci2a_makelocusbed.R: Last row:', last.row, '\n\n')


#### FUNCTIONS -----------------------------------------------------------------
getIndLoci <- function(bedfile,
                       max_dist,
                       last.row = 0) {

  cat("#### vcf2loci2a_makelocusbed.R: getIndLoci function: bedfile:", bedfile, "\n")
  ...
  if(last.row == 0) {
    cat("#### vcf2loci2a_makelocusbed.R: getIndLoci(): Using all rows...\n")
  } else {
    cat("#### vcf2loci2a_makelocusbed.R: getIndLoci(): Selecting rows until row:",
        last.row, "\n")
    bed <- bed[1:last.row, ]
  }
...
cat('\n#### vcf2loci2a_makelocusbed.R: Nr of initial loci:', nrow(bed.merged), '\n')
...
cat('#### vcf2loci2a_makelocusbed.R: Number of loci after filtering by coverage:',
    nrow(bed.cov), '\n\n')

## Trim loci:
cat('#### vcf2loci2a_makelocusbed.R: Trimming loci...\n')
...
cat('#### vcf2loci2a_makelocusbed.R: Trimming done.\n\n')
cat('#### vcf2loci2a_makelocusbed.R: Number of loci retained:', nrow(bed.trim), '\n\n')

## Save bedfile:
cat('#### vcf2loci2a_makelocusbed.R: Writing bedfile', outfile_bed, '\n')
write.table(bed.trim, outfile_bed,
            sep = '\t', quote = FALSE, row.names = FALSE, col.names = FALSE)

## Report:
cat('\n#### vcf2loci2a_makelocusbed.R: Done with script.\n')
